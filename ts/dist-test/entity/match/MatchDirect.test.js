...
(0, node_test_1.describe)('MatchDirect', async () => {
    ...
    (0, node_test_1.test)('direct-exists', async () => {
        const sdk = new __1.FootballDataSDK({
    ...
    (0, node_test_1.test)('direct-load-match', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-match', setup.live))
            return;
...
function liveScenariosActive() { return false && process.env.FOOTBALL_DATA_TEST_LIVE === 'TRUE'; }
function directSetup(mockres) {
    ...
    if (live) {
        ...
        const client = new __1.FootballDataSDK(Object.assign({}, (0, utility_1.liveClientOptions)(), { system: { fetch: transport.fetch },
            apikey: env.FOOTBALL_DATA_APIKEY,
        }));
        let idmap = env['FOOTBALL_DATA_TEST_MATCH_ENTID'];
    ...
    const client = new __1.FootballDataSDK({
        base: 'http://localhost:8080',
