...
//     (\`new FootballDataSDK(...)\` / \`FootballDataSDK.test(...)\` become
//     \`FootballDataSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'FootballDataSDK'
...
const TEST_SEED = {"entity":{"area":{"test01":{"id":"test01"}},"competition":{"test01":{"id":"test01"}},"match":{"test01":{"id":"test01"}},"person":{"test01":{"id":"test01"}},"team":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new FootballDataSDK(...)\` and any
// \`FootballDataSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+FootballDataSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/FootballDataSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
