...
boru-driven command-line client **and** interactive REPL for the FootballData
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/football-data-cli)
make build

# 2. See usage (words, entities, env vars)
./football-data-cli --help

# 3. Provide credentials once, via the environment
export FOOTBALL_DATA_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./football-data-cli list area
./football-data-cli load 1 area            # {id:1} shorthand
./football-data-cli load '{id:1}' area       # explicit match map
./football-data-cli list competition

# 5. Override the API base URL for a single call
FOOTBALL_DATA_BASE=https://api.example.com ./football-data-cli list area

# 6. No arguments -> interactive REPL
./football-data-cli
football-data> list area
football-data> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/football-data-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/football-data-cli list area
   ```
...
```sh
./football-data-cli load 1 area          # scalar shorthand for {id:1}
./football-data-cli load '{id:1}' area     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `football-data>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/football-data-cli`. |
...
(`./football-data-cli <expr>`) and interactively (the REPL), and expressions compose the
