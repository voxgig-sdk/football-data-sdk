...
// FootballData SDK retry feature
...
class FootballDataRetryFeature extends FootballDataBaseFeature
{
    ...
    public function init(FootballDataContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (FootballDataContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(FootballDataContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(FootballDataContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
