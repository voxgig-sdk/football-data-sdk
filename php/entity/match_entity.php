...
class MatchEntity
{
    ...
    {
        if ($args) {
            $this->_data = FootballDataHelpers::to_map(Struct::clone($args)) ?? [];
    ...
    {
        if ($args) {
            $this->_match = FootballDataHelpers::to_map(Struct::clone($args)) ?? [];
     ...
     * Load a single Match.
     *
     * @param MatchLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed MatchLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Match|array The loaded Match as an assoc-array at the
     *   SDK boundary; throws FootballDataError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = FootballDataHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
     ...
     *   the SDK boundary; throws FootballDataError on failure (item-5 convention).
     */
