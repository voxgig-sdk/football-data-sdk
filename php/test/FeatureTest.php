...
class FtCtrl extends FootballDataControl
{
...
class FtClient
{
    ...
    private FootballDataUtility $utility;

    public function __construct(FootballDataUtility $utility, array $options)
    {
    ...
    public function get_utility(): FootballDataUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (FootballDataContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public FootballDataContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (FootballDataContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = FootballDataConfig::make_feature($name);
    ...
    private static function buildUrl(FootballDataSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof FootballDataError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new FootballDataResponse($fetched);
        }
    ...
    private function fail(FootballDataContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (FootballDataContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
