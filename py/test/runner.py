...
class FootballDataTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            FootballDataTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = FootballDataTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = FootballDataTestRunner.getenv("FOOTBALL_DATA_TEST_LIVE")
        override = FootballDataTestRunner.getenv("FOOTBALL_DATA_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = FootballDataTestRunner.getenv(key)
        ...
        explain = FootballDataTestRunner.getenv("FOOTBALL_DATA_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["FOOTBALL_DATA_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if FootballDataTestRunner._test_control is not None:
            return FootballDataTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                FootballDataTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            FootballDataTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = FootballDataTestRunner.load_test_control()
...
def load_env_local():
    FootballDataTestRunner.load_env_local()


def env_override(m):
    return FootballDataTestRunner.env_override(m)


def entity_data(v):
    return FootballDataTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return FootballDataTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return FootballDataTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return FootballDataTestRunner.load_test_control()


def live_client_options():
    return FootballDataTestRunner.live_client_options()


def live_delay_ms():
    return FootballDataTestRunner.live_delay_ms()
