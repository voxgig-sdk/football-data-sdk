...
[MCP](https://modelcontextprotocol.io) server exposing the FootballData SDK as
two agent tools — `football-data_list` and `football-data_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/football-data-mcp)
make build

# 2. Provide credentials via the environment
export FOOTBALL_DATA_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user football-data \
  -- /absolute/path/to/football-data-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./football-data-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// football-data_list: first page of records
{ "entity": "area" }
{ "entity": "area", "query": { } }

// football-data_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/football-data-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user football-data \
     -- "$PWD"/dist/*/football-data-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `football-data_list` and `football-data_load` tools now appear
   in new sessions. Ask the agent to *"list area using football-data"*
   and it calls `football-data_list` with `{"entity":"area"}`.
...
```sh
export FOOTBALL_DATA_APIKEY=sk_live_xxx            # API key
export FOOTBALL_DATA_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `football-data_list` tool
...
### Call the `football-data_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `football-data_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `football-data_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `FOOTBALL_DATA_APIKEY` | API key sent with every request. |
| `FOOTBALL_DATA_BASE` | Optional override of the API base URL. |

### Entities

The 5 entities valid as the `entity` argument:

area | competition | match | person | team

### Smoke test via HTTP (raw JSON-RPC)

```sh
./football-data-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"football-data_load","arguments":{"entity":"area","query":{"id":1}}}}'
```
