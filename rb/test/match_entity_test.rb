...
class MatchEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    base = FootballDataSDK.test(seed, nil)
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set FOOTBALL_DATA_TEST_MATCH_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    match_ref01_data_dt0_load_result = Helpers.to_map(match_ref01_data_dt0_loaded.respond_to?(:data_get) ? match_ref01_data_dt0_loaded.data_get : match_ref01_data_dt0_loaded)
...
def match_basic_setup(extra)
  ...
  entid_env_raw = ENV["FOOTBALL_DATA_TEST_MATCH_ENTID"]
