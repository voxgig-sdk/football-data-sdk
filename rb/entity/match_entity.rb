...
class MatchEntity
  ...
  def data_set(args)
    if args
      @_data = FootballDataHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = FootballDataHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # Load a single Match.
  #
  # @param reqmatch [MatchLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Match.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Match, Hash] the loaded Match; raises FootballDataError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = FootballDataHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Match>, Array] the matching Match items; raises FootballDataError on failure
