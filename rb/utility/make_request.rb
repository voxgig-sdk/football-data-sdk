...
module FootballDataUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = FootballDataResponse.new({})
    ...
    if fetch_err
      ...
      response = FootballDataResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = FootballDataResponse.new(fetched)
    else
