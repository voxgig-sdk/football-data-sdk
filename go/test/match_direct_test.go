...
func TestMatchDirect(t *testing.T) {
	...
	t.Run("direct-load-match", func(t *testing.T) {
		setup := matchDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-match", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
func matchDirectSetup(mockres any) *matchDirectSetupResult {
	...
	env := envOverride(map[string]any{
		"FOOTBALL_DATA_TEST_MATCH_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["FOOTBALL_DATA_TEST_MATCH_ENTID"]; ok {
