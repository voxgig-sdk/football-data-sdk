...
func TestTeamEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set FOOTBALLDATA_TEST_TEAM_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func teamBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("FOOTBALLDATA_TEST_TEAM_ENTID")
	...
	idmapResolved := core.ToMapAny(env["FOOTBALLDATA_TEST_TEAM_ENTID"])
